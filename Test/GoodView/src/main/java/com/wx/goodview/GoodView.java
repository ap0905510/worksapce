...
public class GoodView extends PopupWindow implements IGoodView {
    ...
    private String mText = TEXT; //动画显示的文本

    private int mTextColor = TEXT_COLOR; //文字的颜色

    private int mTextSize = TEXT_SIZE; //文字大小

    private int mFromY = FROM_Y_DELTA; //动画Y轴的初始位置

    private int mToY = TO_Y_DELTA; //动画X轴的结束位置

    private float mFromAlpha = FROM_ALPHA; //Alpha开始透明度 1.0f

    private float mToAlpha = TO_ALPHA; //Alpha结束透明度 0.0f

    private int mDuration = DURATION; //动画时间 800ms

    private int mDistance = DISTANCE; //动画移动的最大距离 60

    private AnimationSet mAnimationSet; //动画对象

    private boolean mChanged = false; //是否改变

    private Context mContext = null; //上下文

    private TextView mGood = null; //显示动画TextView
